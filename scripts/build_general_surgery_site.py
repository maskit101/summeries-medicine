...
# -*- coding: utf-8 -*-
"""
בונה מחדש את docs/general-surgery/index.html, docs/general-surgery/summaries.html,
ומעדכן את הריבוע של כירורגיה כללית ב-docs/index.html (עמוד הבית הראשי),
לפי general_surgery_state.json ו-docs/general-surgery/flashcards_all.json.

הרץ את הסקריפט הזה בכל פעם שסטטוס פרק משתנה ב-general_surgery_state.json
(אחרי שנוצרו סיכום/כרטיסיות/PPTX חדשים לפרק).

שימוש: python3 scripts/build_general_surgery_site.py
...
STATE_PATH = os.path.join(BASE, "general_surgery_state.json")
NS_DIR = os.path.join(BASE, "docs", "general-surgery")
...
<title>סיכומי כירורגיה כללית - Sabiston/Schwartz</title>
<style>
...
<div class="layout">
  <nav class="sidebar" id="sidebar">
    ...
    <div class="sidebar-inner">
      <h2>Sabiston & Schwartz Principles of Surgery</h2>
      ...
      <div class="top-links">
        ...
        <a class="home" href="index.html">&#8592; עמוד כירורגיה כללית</a>
...
'''
    ...
    # ---------- general-surgery/index.html (2-tile hub) ----------
    index_html = f'''<!DOCTYPE html>
<html lang="he" dir="rtl">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>כירורגיה כללית - Sabiston/Schwartz</title>
<style>
  :root{{color-scheme:light}}
  *{{box-sizing:border-box}}
...
'''
    ...
    # ---------- update the gs (general-surgery) tile stats on the root docs/index.html ----------
    if os.path.exists(ROOT_INDEX_PATH):
        with open(ROOT_INDEX_PATH, encoding="utf-8") as f:
            root_html = f.read()
        root_html = re.sub(
            r'(<a class="tile gs"[^>]*>.*?<div class="stat"><b>)\d+/\d+(</b>פרקים</div>\s*<div class="stat"><b>)\d+(</b>כרטיסיות</div>)',
    ...
    print(f"Built general-surgery site — {len(done)}/{len(chapters)} chapters done, {num_cards} cards.")
