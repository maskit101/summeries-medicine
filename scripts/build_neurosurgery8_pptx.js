// -*- coding: utf-8 -*-
// בונה PPTX לפרק נוירוכירורגיה בודד (מהדורה 8) מתוך neurosurgery8_state.json + docs/neurosurgery8/flashcards_all.json.
// שימוש:  node scripts/build_neurosurgery8_pptx.js <chapter_n>   (chapter_n יכול להיות "100" או "94A")
// דורש: npm install pptxgenjs (בתיקייה שממנה מריצים, או צריך node_modules נגיש)
// הפלט נשמר ל-docs/neurosurgery8/summaries/<NN>-<slug>.pptx

const pptxgen = require("pptxgenjs");
const fs = require("fs");
const path = require("path");

const chapterN = process.argv[2];
if (!chapterN) {
  console.error("Usage: node build_neurosurgery8_pptx.js <chapter_n>");
  process.exit(1);
}

const REPO = path.join(__dirname, "..");
const VOL_DIR = path.join(REPO, "docs/neurosurgery8");
const state = JSON.parse(fs.readFileSync(path.join(REPO, "neurosurgery8_state.json"), "utf8"));
const chapter = state.chapters.find(c => String(c.n) === String(chapterN));
...
function slug(n, titleEn) {
  ...
  let nn = String(n);
  if (/^\d+$/.test(nn)) nn = nn.padStart(2, "0");
  return nn + "-" + s;
}
...
// ---- Title slide ----
{
  ...
  slide.addText("נוירוכירורגיה — מהדורה 8", { x: 1, y: 2.3, w: 11.33, h: 1.0, align: "center", fontSize: 40, bold: true, color: WHITE, fontFace: "Calibri", rtlMode: true });
...
// ---- Closing slide (generic — lists key_terms across all this chapter's cards as a recap) ----
{
  ...
  slide.addText("maskit101.github.io/summeries-medicine/neurosurgery8", { x: 1, y: 6.9, w: 11.33, h: 0.4, align: "center", fontSize: 12, color: "64748B", fontFace: "Calibri" });
}
