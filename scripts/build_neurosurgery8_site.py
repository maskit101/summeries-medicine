...
# -*- coding: utf-8 -*-
"""
בונה מחדש את אתר "נוירוכירורגיה - מהדורה 8" (docs/neurosurgery8/summaries.html + index.html)
לפי neurosurgery8_state.json ו-docs/neurosurgery8/flashcards_all.json, ומעדכן את הריבוע
המתאים ב-docs/index.html (עמוד הבית הראשי).

פרויקט עצמאי לגמרי מ-docs/neurosurgery (מהדורה 6, 4 כרכים) - לא נוגע בו.
מקור: תיקייה שטוחה יחידה עם קובץ PDF אחד לכל פרק (Chapter_NN[A/B]_Title.pdf),
ללא חלוקה לכרכים. מספרי פרקים יכולים לכלול סיומת אות (למשל "94A", "94B").

שימוש: python3 scripts/build_neurosurgery8_site.py
"""
import json, re, os

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
BASE = os.path.dirname(SCRIPT_DIR)  # repo root
SITE_DIR = os.path.join(BASE, "docs", "neurosurgery8")
STATE_PATH = os.path.join(BASE, "neurosurgery8_state.json")
ROOT_INDEX_PATH = os.path.join(BASE, "docs", "index.html")
BOOK_LABEL = "Youmans Neurological Surgery, 8th Edition"
...
def main():
    os.makedirs(os.path.join(SITE_DIR, "summaries"), exist_ok=True)

    with open(STATE_PATH, encoding="utf-8") as f:
        state = json.load(f)
    chapters = sorted(state["chapters"], key=sort_key)
    book = state.get("book", BOOK_LABEL)
    ...
    fc_path = os.path.join(SITE_DIR, "flashcards_all.json")
...
<title>סיכומי נוירוכירורגיה - מהדורה 8 - Youmans</title>
<style>
  ...
  *{{box-sizing:border-box}}
  ...
  .ch-num{{width:30px; height:24px; border-radius:12px; background:#374151; color:#fff; font-size:11px; display:flex; align-items:center; justify-content:center; flex-shrink:0}}
...
<div class="layout">
  <nav class="sidebar" id="sidebar">
    ...
    <div class="sidebar-inner">
      <h2>Youmans Neurological Surgery — מהדורה 8</h2>
      ...
      <div class="top-links">
        ...
        <a class="home" href="index.html">נוירוכירורגיה: מהדורה 8</a>
  ...
  <main>
    ...
    <div class="empty-wrap" id="emptyWrap">
      <h1>סיכומי נוירוכירורגיה — מהדורה 8</h1>
      ...
      <div class="latest">
        ...
        <a href="flashcards.html" style="margin-top:6px">🗂️ כל הכרטיסיות — מאגר מאוחד אחד</a>
      </div>

      <p style="color:#6b7280; font-size:14px">כל יום מתווסף כאן סיכום וחבילת כרטיסיות לפרק הבא במהדורה 8, לפי סדר מספרי הפרקים. השתמשי בתפריט הצד או בכפתורי "הבא/הקודם" כדי לעבור בין הפרקים שכבר הושלמו — התפריט נשאר גלוי כל הזמן. רשימת הפרקים מתעדכנת אוטומטית כשמועלים עוד קבצי PDF לתיקיית המקור.</p>
    </div>
  </main>
</div>
...
document.querySelectorAll('.ch-btn').forEach(function(btn){{
  btn.addEventListener('click', function(){{ openChapter(btn.getAttribute('data-n')); }});
}});

var sidebarEl = document.getElementById('sidebar');
var sidebarToggle = document.getElementById('sidebarToggle');
var COLLAPSE_KEY = 'ns8_sidebar_collapsed';
...
'''
    with open(os.path.join(SITE_DIR, "summaries.html"), "w", encoding="utf-8") as f:
        f.write(summaries_html)

    # ---------- index.html (2-tile mini hub) ----------
    index_html = f'''<!DOCTYPE html>
<html lang="he" dir="rtl">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>נוירוכירורגיה — מהדורה 8 - Youmans</title>
<style>
  :root{{color-scheme:light}}
  *{{box-sizing:border-box}}
  ...
  .tile.summaries{{background:linear-gradient(155deg, #0891b2, #0c1b2a)}}
...
<div class="grid">
  <a class="tile summaries" href="summaries.html">
    ...
    <h2>סיכומים</h2>
    <p>סיכום מעמיק לכל פרק במהדורה 8, עם שאלון תרגול — בחירה מתפריט צד לפי פרק</p>
    <div class="stat"><b>{len(done)}/{len(chapters)}</b> פרקים</div>
  </a>
  <a class="tile flashcards" href="flashcards.html">
    <span class="icon">🗂️</span>
    <h2>כרטיסיות</h2>
    <p>בנק כרטיסיות תרגול מאוחד לכל פרקי מהדורה 8, עם סינון, שאלון וסטטיסטיקות</p>
...
'''
    ...
    # ---------- update the neuro8 tile stats on the root docs/index.html ----------
    if os.path.exists(ROOT_INDEX_PATH):
        with open(ROOT_INDEX_PATH, encoding="utf-8") as f:
            root_html = f.read()
        pattern = re.compile(
            r'(<a class="tile neuro8"[^>]*>.*?<div class="stat"><b>)\d+/\d+(</b>פרקים</div>\s*<div class="stat"><b>)\d+(</b>כרטיסיות</div>)',
            flags=re.S
        )
        if pattern.search(root_html):
            new_root_html = pattern.sub(
            ...
            print("  NOTE: root docs/index.html neuro8 tile pattern not matched — check manually.")

    print(f"Built neurosurgery8 site — {len(done)}/{len(chapters)} chapters, {num_cards} cards.")
